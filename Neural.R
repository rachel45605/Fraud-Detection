...
new<-subset(fraud_data, select = -c(DayOfWeekClaimed,PastNumberOfClaims) )

#normalise using one hot encoding 
newdata <- one_hot(as.data.table(new))
dim(newdata)
...
newdata <- as.data.frame(lapply(newdata, normalize))
View(newdata)


# ==============================================================================================================
###create train and test set####
names(newdata) <- make.names(names(newdata)) #Make syntactically valid names out of character vectors
train = sample.split(Y = newdata$FraudFound_P, SplitRatio=0.7)
trainset = subset(newdata,train == T)
testset = subset(newdata, train == F)
...
m1 <- neuralnet(FraudFound_P ~ . , data=trainset.bal, hidden=c(20,15), err.fct="ce", linear.output=FALSE)
par(mfrow=c(1,1))
plot(m1)
...
threshold <- 0.5
y.hat <- as.factor(ifelse(out$V89 > threshold, 1, 0))
y.hat
...
pr.m1_ <- pr.m1$net.result*(max(newdata$FraudFound_P)-min(newdata$FraudFound_P))+min(newdata$FraudFound_P)
test.r <- (testset$FraudFound_P)*(max(newdata$FraudFound_P)-min(newdata$FraudFound_P))+min(newdata$FraudFound_P)
